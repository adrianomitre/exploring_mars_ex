defmodule Parser do
  ...
  defp validate_syntax_for_position_with_optional_direction(line) do
    unless String.match?(line, ~r/^\s*\d+\s\d+(?:\s+[ENWS])?\s*$/) do
      # ArgumentError
      raise "invalid syntax for position w/ opt. direction"
    end
